...
- 스타일과 크기를 선택한 후 [이미지 생성하기] 버튼을 누르면 결과가 출력됩니다.
""")
...
#이미지 크기선택
size_label = st.selectbox("📏 이미지 크기 선택", ["작게 (256x256)", "보통 (512x512)", "크게 (1024x1024)"])
size_map = {
    "작게 (256x256)": "256x256",
    "보통 (512x512)": "512x512",
    "크게 (1024x1024)": "1024x1024"
}
    ...
    with st.spinner("이미지를 생성 중입니다..."):
        try:
            response = client.images.generate(
                prompt=final_prompt,
                model="dall-e-3",  # DALL·E 3는 n=1만 가능
                n=1,
                size="1024x1024"
            )
